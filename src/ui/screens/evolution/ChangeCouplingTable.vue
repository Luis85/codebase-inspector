...
defineProps<{ rows: readonly CouplingRow[] }>();
...
/** The inline bar's length; an unknown rate draws no bar at all (never a 0-length "value"). */
function rateWidth(row: CouplingRow): string | null {
  return hasValue(row.rate) ? `${Math.min(100, Math.max(0, row.rate.value))}%` : null;
}
</script>
      ...
      <template #cell-rate="{ row }">
        <span class="ci-coupling__rate-cell">
          ...
          ><span
            ...
            :style="{ inlineSize: rateWidth(row) ?? undefined }"
          /></span>
