<!--
  ...
  CityRelationsPanel — shown only while a report is attached, headed by the same
  EvidenceBadge Quality and File detail use (review fix round 1, item 6), for the
  selected file's own `touchingFindings` (findings.ts): anchored findings first, then
  related ones (marked FINDING_VIA_RELATED), at most CITY_FINDINGS_LIMIT rows, then "N
  more on Investigate". A report attached but nothing touching this file reads
  FILE_NO_FINDINGS_REPORTED (Y36, review fix round 1, item 5) — never a bare heading over
  an empty list. Every row's Investigate button goes through the ONE entry point
...
import {
  CITY_FINDINGS_MORE, CITY_FINDINGS_ROW, CITY_FINDINGS_TITLE, FILE_NO_FINDINGS_REPORTED, FINDING_KIND_LABEL, FINDING_VIA_RELATED,
  INVESTIGATE_ACTION, INVESTIGATE_FINDING_LABEL, RULE_TEXT,
} from '../../inspector-copy';
...
/** Review fix round 1, item 6: the same EvidenceBadge Quality and File detail head their
 *  findings with, so a stale report reads the same way everywhere it is shown. */
const badge = computed(() => evidenceBadgeFor(evidence.value));
...
<template>
  ...
  >
    ...
    <!-- Review fix round 1, item 5 (Y36): a report attached but nothing touching this file
         is not zero complexity — FILE_NO_FINDINGS_REPORTED, never a bare heading over an
         empty <ul>. -->
    <p
      v-if="rows.length === 0"
      class="ci-note ci-city-findings__none"
    >
