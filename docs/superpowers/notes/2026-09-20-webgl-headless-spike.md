...
Notably, the `default` variant (no launch args at all) already succeeds on this machine. The probe did not test why; the likely explanation is that this Chromium build's default GPU policy falls back to Vulkan SwiftShader in a headless/no-GPU context without needing explicit flags, but that is an inference from the observed renderer string, not something the probe measured directly. Whatever the mechanism, it is an implementation detail of Chromium's auto-detection (dependent on what it discovers about available GPU/ICD support at launch), not a documented guarantee. The two SwiftShader-forcing variants produce the exact same renderer and pixel result while removing that ambiguity — they say directly "use the software path" instead of relying on Chromium to arrive there on its own. For a harness that has to work unattended on machines whose GPU/driver state we don't control, the explicit variant is the safer contract to depend on.
