...
**"Read-only source access"** and **"Scanning never changes the source."** (reworded by
WP-04 ruling E29; see that part's G2 note in the gate-evidence document) ship at two sites:
...
**Native coverage is not a G8 layer (IP53).** `npm run test:e2e` drives a real installed
Obsidian and is opt-in, local and never part of `npm run verify`. After WP-04 Part 2 its
results gate requires 36 scenarios across 13 files (`smoke`, `obsidian-facts`, `lifecycle`,
`investigation`, `settings`, `plugin-lifecycle`, `commands`, `fallow`, `notes-index`,
`notes-refresh`, `notes-root`, `preview`, `city`). At commit `3275092` on this Windows 11
machine, both the baseline run (Obsidian 1.13.4) and one `OBSIDIAN_VERSION=latest` run
(resolved to 1.13.7) printed `Verified 36 executed native Vitest cases, including all 36
required scenarios.` The commands, per-file timings, resolved versions and the scenario
list by file are in the gate-evidence document's **WP-04 Part 2** section. There is no CI
for it (owner decision O3).
...
- `Scanning never changes the source.` — the second claim, at both the same places
  (reworded by WP-04 ruling E29, once O7 allowed a confirmed note inside the root).
