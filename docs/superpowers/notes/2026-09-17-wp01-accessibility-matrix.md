...
| Reduced motion: camera jumps, does not tween | jsdom + manual | **jsdom half PASSED.** `setMotion('reduced')` is passed when `prefers-reduced-motion` matches, read from `containerEl.win` and re-read on every reconstruction; the rig honours it. The LIVE OS-level half is machine-checked as of the final fix wave, and was not before: every `matchMedia` double in the tree registered the change listener into a spy that could never fire one, so deleting the live response from `applyMotionPreference` left the whole suite green — release was tested, response was not. `tests/component/city-viewport.test.ts` now keeps the listener and fires it ("follows a LIVE prefers-reduced-motion change"), asserting the preference is re-read both ways; deleting the live response reddens exactly that test. **What the double still cannot establish is that a REAL OS-level preference change reaches the page**, which is a host behaviour, not ours. **Manual half NOT PERFORMED: whether the camera visibly jumps rather than tweening, which requires watching it.** |
