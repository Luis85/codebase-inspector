...
| Dark theme | manual | **NOT PERFORMED** as an accessibility check. The user has run the plugin in their own theme and closed checkpoint #3 on a screenshot, but no contrast measurement was taken. Adjacent: every scene colour is re-read from the theme's tokens on `css-change` (acceptance "Theme change while a city is open"). |
| Light theme | manual | **NOT PERFORMED.** Same as above; no light-theme run is recorded anywhere on this branch. |
