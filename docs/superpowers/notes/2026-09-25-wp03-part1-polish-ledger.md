...
| WP-03 Polish E8 | **Ruling:** the controller's closing `verify` found `settings-tab.test.ts`'s whole-`src` `.obsidian` scan without an explicit timeout (it hit the 5 s default under load); it gets `30_000` on its existing line (f44d735). The Z38 no-freeze case missed its budget once more under the same load (50.9 ms) and passed alone three times (20–41 ms); the next full `verify` exited 0. — The explicit-timeout rule for whole-`src` scans binds every part; the no-freeze path is untouched by this pass (E3). — Low: the no-freeze budget stays load-sensitive until Z38 is taken up. |
