...
reads every `.ts`/`.vue` file under `src/` (112+ files, count asserted so the sweep cannot
...
**112 files, 1074 tests, 1073 passed,
1 skipped.**
...
|---|---|---|---|---|---|
| Unit | `tests/unit/**` | 47 | yes | 504 | domain, application, UI stores, interaction state, stylesheet-as-contract (comments stripped — see below), and this table's own guard |
...
| Component (jsdom) | `tests/component/**` | 39 | yes | 340 | against **our** controls: the file list, search, inspector, camera controls, viewport, status surfaces, announcements, both modals, the settings tab, the renderer contract and disposal, (task 5) the toolbar's Scan control, and (task 7) the canvas header |
