...
| Y18 | Clear, import and export | `clearAll` and `replaceAll` already go through the port, so they become durable with no change. Export v2 is unchanged. Import still replaces, never merges (T12). |
...
| `src/adapters/fallow/fallow-report-schema.ts` | new | Y22 |
| `src/adapters/fallow/read-fallow-report.ts` | new | reading the file, the size limit, JSON parsing, the schema, errors (Y20, Y31) |
...
| `src/ui/components/MetricLegend.vue`, `src/ui/screens/city/LensHeading.vue` (new), `CityViewport` | edit/new | Y40 |
