# These are acceptance specifications, not executed Cucumber/Pester results.
# The actual test execution records are model-test-results.tap and browser-results.json.
@wp01
Feature: Inspect a structural city without losing evidence or user context
  ...
  Scenario: Escape in search clears only the query
    Given a file is selected and the search field has a nonempty query
  ...
  Scenario: Keyboard input belongs to the sibling note
    Given Codebase Inspector and a Markdown note are visible
  ...
  Scenario: Restore a 3D camera after top-view exploration
    Given a nondefault 3D camera bookmark
  ...
  Scenario: Return to the scope review trigger
    Given I opened scope review from the Scan action
    When I close the modal using Escape
  ...
  Scenario: Respect the approved source boundary
    Given a source contains a symlink outside the approved root
