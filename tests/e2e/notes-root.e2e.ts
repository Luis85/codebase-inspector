...
import { closeSettings, pluginData, reloadPlugin } from './host-probes';
...
const profileOf = async (browser: NativeContext['browser']): Promise<SavedProfile> => {
  const saved = ((await pluginData(browser)).profiles ?? []) as SavedProfile[];
...
/** The vault base path the plugin reads (`FileSystemAdapter.getBasePath()`, as investigation-notes.ts does). */
const obsidianBasePath = (browser: NativeContext['browser']): Promise<string | null> => browser.executeObsidian(({ app, obsidian }) => {
...
describe('notes inside the codebase root (WP-04.2 rows 20, 22)', () => {
  test('a note created inside the codebase root is excluded from the next scan', async ({ native }) => {
    ...
    const exclusions = (await profileOf(browser)).exclusions;
    ...
    await expect.poll(async () => (await profileOf(browser)).exclusions).toEqual([...exclusions, 'notes']);
    ...
    expect((await profileOf(browser)).exclusions).toEqual([...exclusions, 'notes']);
    await inspector.rescan();
    const grown = await storeSnapshot(browser);
    await writeEvidence(directory, 'in-root', {
      excludedNote, keptNote, exclusions: (await profileOf(browser)).exclusions,
  ...
  test('offers the scan exclusion when the codebase root is reached through an alias', async ({ native }) => {
    ...
    const base = await obsidianBasePath(browser);
    ...
    expect((await profileOf(browser)).bindingId).toBe(bindings[0]?.bindingId);
